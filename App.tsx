...
import { 
  Settings, FileText, User, Save, Trash2, Upload, Download, 
  Loader2, CheckCircle2, AlertCircle, FileSpreadsheet, 
  DownloadCloud, UserPlus, X, FileUp, Info, Lock, LogOut, Key, ShieldCheck
} from 'lucide-react';
...
// Zakódované prístupové údaje (Meno: okrzv, Heslo: zvokr)
...
const App: React.FC = () => {
  ...
  // Auth checking and settings loading
  useEffect(() => {
    // Kontrola, či je používateľ už prihlásený (session v LocalStorage)
    const session = localStorage.getItem('pohotovost_session_active');
    if (session === 'true') {
  ...
  const handleLogin = (e: React.FormEvent) => {
    e.preventDefault();
    // Porovnanie zadaných údajov s tými v kóde (zakódovanými)
    if (btoa(loginUsername) === AUTH_CREDENTIALS.u && btoa(loginPassword) === AUTH_CREDENTIALS.p) {
      setIsAuthenticated(true);
      localStorage.setItem('pohotovost_session_active', 'true');
      setAuthError(null);
    } else {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem('pohotovost_session_active');
    setLoginUsername('');
    setLoginPassword('');
    setAuthError(null);
  };
  ...
  if (!isAuthenticated && !isAuthLoading) {
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-6 bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-green-100 via-slate-50 to-blue-50">
        <div className="max-w-md w-full animate-in fade-in zoom-in duration-500">
          <div className="bg-white rounded-[2.5rem] shadow-2xl shadow-slate-200 border border-slate-200 overflow-hidden relative">
            ...
            <div className="p-12 text-center">
              <div className="bg-green-600 w-20 h-20 rounded-3xl flex items-center justify-center mx-auto mb-8 shadow-xl shadow-green-100 animate-bounce">
                <Lock className="w-10 h-10 text-white" />
              </div>
              
              <h1 className="text-3xl font-black text-slate-900 mb-2">Prístup do systému</h1>
              <p className="text-slate-500 font-bold text-sm mb-10 uppercase tracking-widest leading-relaxed">
                Zadajte overovacie údaje pre pokračovanie
              </p>

              <form onSubmit={handleLogin} className="space-y-6 text-left">
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Používateľské meno</label>
                  <div className="relative group">
                    ...
                    <input 
                      ...
                      className="w-full pl-14 pr-6 py-4 rounded-2xl border-2 border-slate-100 focus:border-green-500 focus:ring-0 outline-none transition-all font-bold text-slate-800 bg-slate-50 focus:bg-white"
                      placeholder="Meno"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] ml-1">Heslo</label>
                  <div className="relative group">
                    <Key className="absolute left-5 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-green-500 transition-colors" />
                    <input 
                      type="password" 
                      value={loginPassword}
                ...
                {authError && (
                  <div className="bg-red-50 text-red-600 p-4 rounded-2xl text-xs font-black flex items-center gap-3 animate-shake">
                ...
                <button 
                  type="submit"
                  className="w-full py-5 bg-slate-900 text-white rounded-2xl font-black text-lg shadow-xl shadow-slate-200 hover:bg-black hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-3"
                >
                  <ShieldCheck className="w-5 h-5" /> Vstúpiť do aplikácie
                </button>
              </form>

              <p className="mt-8 text-[10px] text-slate-400 font-bold uppercase tracking-widest leading-relaxed">
                Prístup je povolený len pre zamestnancov OU Zvolen.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          <nav className="flex gap-1 bg-slate-100 p-1.5 rounded-2xl">
            <button
              ...
              className={`px-4 sm:px-6 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'generate' ? 'bg-white text-green-600 shadow-md ring-1 ring-slate-200' : 'text-slate-500 hover:text-slate-900'}`}
            >
              Generovanie
            </button>
            <button
              onClick={() => setActiveTab('settings')}
              className={`px-4 sm:px-6 py-2 rounded-xl text-sm font-bold transition-all ${activeTab === 'settings' ? 'bg-white text-green-600 shadow-md ring-1 ring-slate-200' : 'text-slate-500 hover:text-slate-900'}`}
            >
              Nastavenia
            </button>
          </nav>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-2">
                        ...
                        <input 
                          ...
                          className="w-full px-5 py-3.5 rounded-2xl border-2 border-slate-100 focus:border-green-500 focus:ring-0 outline-none transition-all text-sm font-black text-slate-800 bg-slate-50/50"
                        />
                      </div>
                      <div className="space-y-2">
